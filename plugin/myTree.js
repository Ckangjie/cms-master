layui.define(['jquery'], function(exports) {
	...
	tree.prototype.config = function() {
		return {
			...
			checked: {},
			is_click_icon: false,
			is_cache: false, 		// 存储折叠状态
			is_checkbox: true, 		// 开启复选框
			is_check_auto: true, 	// 选择父级时，智能选择子级
			is_radio_last: false 	// 下拉选择只选择最后一级
		};
	}
	tree.prototype.templet = function(e) {
		var t = this,
			level = [],
			tbody = '',
		...
		o.each(t.data(e, e.data), function(idx, item) {
			...
			o.each(e.cols, function(index, obj) {
				...
				// 拼接行
				tr += '<td ' + style + (left ? 'data-down' : '') + '>' + icon + (is_table ? '' : (is_checked ? checked : checkbox)) + (obj.templet ? obj.templet(item) : item[obj.key]) + '</td>';
			});
		...
		o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function() {
			...
			// @Author: 魏大毛 [1920660234@qq.com]
			// 原列表操作监听事件为：<a lay-filter="add">操作</a> 现更改为：lay-event监听
		}).off('click', '[lay-event]').on('click', '[lay-event]', function() {
			var tr = o(this).parents('[data-id]'),
				id = tr.data('id'),
				pid = tr.data('pid'),
				event = o(this).attr("lay-event");
			return layui.event.call(this, MOD_NAME, 'tree(' + event + ')', {
				elem: this,
				item: e.childs[pid][id],
			})
		});
	...
	// 获取已选ID集合
	tree.prototype.checkedId = function(e) {
		var ids = [];
	...
	tree.prototype.checked = function(e) {
		var checkedData = [], ids = this.checkedId(e);
		// 循环已选ids
		for(var i in ids){
			// 循环所有数据
			for(var j in e.data){
				if(ids[i] == e.data[j].id) checkedData.push(e.data[j]);
			}
		}
		return checkedData;
	};
	// @Author: 魏大毛 [1920660234@qq.com]
	// 下拉选择
	tree.prototype.treeDown = function(e){
		if(!e.is_checkbox){
			var obj_elem = o(e.elem),
				obj_tree = obj_elem.parents('.tree'),
				obj_anim = obj_elem.parent('.layui-anim'),
				obj_down = obj_tree.children('.layui-select-title').children('.layui-unselect'),
				obj_input = obj_down.next('input[type="hidden"]');

			// 编辑时选中当前的父级
			var pid = e.checked.data;
			if(pid){
				for(var i in e.data){
					if(pid == e.data[i].id) obj_down.val(e.data[i].name);
				}
			}
			// 点击下拉
			obj_down.click(function(){
				if(obj_tree.hasClass('layui-form-selected')){
					obj_tree.removeClass('layui-form-selected');
				}else{
					obj_tree.addClass('layui-form-selected');
				}
			});
			// 清空
			o('.layui-select-tips').click(function(){
				obj_input.val(0);
				obj_down.val('');
				obj_tree.removeClass('layui-form-selected');
			});
			// 选择
			obj_elem.on('click','ul li',function(el){
				if(el.target.tagName == 'LI'){
					if(!e.is_radio_last){ 	// 所有都可选择
						downClick(o(this));
					}else{ 					// 只选择最后一级
						if(!o(this).children('i').length){
							downClick(o(this));
						}else{
							o(this).children('i').click();
							return false;
						}
					}
				}else{
			...
			function downClick(obj){
				obj_input.val(obj.parent('ul').data('id'));
				obj_down.val(obj.text());
				obj_tree.removeClass('layui-form-selected');
			}
		}
	};
